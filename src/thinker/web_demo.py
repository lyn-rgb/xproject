...
def _trigger_avatar_generation(args, prompt, audio_path):
    if not args.avatar_server:
        return
    payload = {
        "cond_image_path": args.avatar_cond_image,
        "audio_path": audio_path,
        "input_prompt": prompt,
        "audio_encode_mode": args.avatar_audio_encode_mode,
        "base_seed": args.avatar_base_seed,
    }
    try:
        resp = requests.post(
            f"{args.avatar_server.rstrip('/')}/start",
            json=payload,
            timeout=5,
        )
        resp.raise_for_status()
    except Exception as exc:
        print(f"Avatar trigger failed: {exc}")
...
def _launch_demo(args, model, processor):
    ...
    def predict(messages, voice=DEFAULT_VOICE):
        ...
        _trigger_avatar_generation(args, response, audio_path)
...
def _get_args():
    ...
    parser.add_argument('--avatar-server', type=str, default='', help='Avatar WebRTC server URL, e.g. http://127.0.0.1:7861')
    parser.add_argument('--avatar-cond-image', type=str, default='examples/man.png', help='Condition image path for avatar.')
    parser.add_argument('--avatar-audio-encode-mode', type=str, default='stream', choices=['stream', 'once'], help='Audio encode mode for avatar.')
    parser.add_argument('--avatar-base-seed', type=int, default=9999, help='Base seed for avatar generation.')
